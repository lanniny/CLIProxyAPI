...
import { IconBot, IconDownload, IconInfo, IconTrash2, IconEdit } from '@/components/ui/icons';
...
export function AuthFilesPage() {
  ...
  const [weightFile, setWeightFile] = useState<AuthFileItem | null>(null);
  ...
  // 打开权重编辑弹窗
  const openWeightModal = (file: AuthFileItem) => {
    setWeightFile(file);
    setWeightValue(file.weight !== undefined && file.weight !== null ? String(file.weight) : '');
  ...
  const saveWeight = async () => {
    ...
    const weight = weightValue.trim() === '' ? undefined : parseInt(weightValue, 10);
    if (weightValue.trim() !== '' && (isNaN(weight!) || weight! < 0)) {
      showNotification(t('auth_files.weight_invalid'), 'error');
      return;
    }
    setSavingWeight(true);
    try {
      await authFilesApi.patchAuthFile(weightFile.name, { weight: weight ?? 0 });
      showNotification(t('auth_files.weight_save_success'), 'success');
      setWeightModalOpen(false);
      // 更新本地状态
      setFiles((prev) =>
        prev.map((f) => (f.name === weightFile.name ? { ...f, weight: weight ?? 0 } : f))
      );
    } catch (err: unknown) {
      const errorMessage = err instanceof Error ? err.message : '';
      showNotification(`${t('auth_files.weight_save_failed')}: ${errorMessage}`, 'error');
    } finally {
      ...
      <div key={item.name} className={styles.fileCard}>
        ...
        <div className={styles.cardMeta}>
          ...
          {item.weight !== undefined && item.weight > 0 && (
            <span className={styles.weightBadge}>{t('auth_files.weight_label')}: {item.weight}</span>
          )}
        </div>
          ...
          {!isRuntimeOnly && (
            <>
              <Button
                ...
                className={styles.iconButton}
                title={t('auth_files.weight_button')}
                disabled={disableControls}
              >
                <IconEdit className={styles.actionIcon} size={16} />
              </Button>
              <Button
                variant="secondary"
                size="sm"
  ...
  return (
    <div className={styles.container}>
      ...
      <Modal
        ...
        title={t('auth_files.weight_modal_title', { name: weightFile?.name || '' })}
        footer={
          <>
            <Button variant="secondary" onClick={() => setWeightModalOpen(false)} disabled={savingWeight}>
              {t('common.cancel')}
            </Button>
      ...
      >
        <div className={styles.formGroup}>
          <label>{t('auth_files.weight_label')}</label>
          <Input
            type="number"
            min={0}
            value={weightValue}
          ...
          />
          <div className={styles.hint}>{t('auth_files.weight_hint')}</div>
        </div>
      </Modal>
    </div>
  );
