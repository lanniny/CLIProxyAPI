...
export interface RoutingStrategy {
  strategy: 'round-robin' | 'fill-first' | 'weighted-round-robin' | 'latency-aware' | 'adaptive';
}
...
export const loadBalancingApi = {
   ...
   */
  async getRoutingStrategy(): Promise<RoutingStrategyResponse> {
    return await apiClient.get<RoutingStrategyResponse>('/routing-strategy');
  },

  /**
   * Update routing strategy
   */
  async updateRoutingStrategy(strategy: RoutingStrategy['strategy']): Promise<void> {
    await apiClient.put('/routing-strategy', { strategy });
  },

  /**
   * Trigger manual health check for a credential
   */
  async triggerHealthCheck(authId: string): Promise<void> {
    await apiClient.post(`/health-check/trigger/${encodeURIComponent(authId)}`);
  },

  /**
   * Reset health status for a credential
   */
  async resetHealthStatus(authId: string): Promise<void> {
    await apiClient.post(`/health-status/${encodeURIComponent(authId)}/reset`);
  },
