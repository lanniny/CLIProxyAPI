...
export const authFilesApi = {
  ...
  // 更新认证凭证属性（如权重）
  async patchAuthFile(name: string, updates: { weight?: number }): Promise<{ status: string; weight?: number }> {
    return apiClient.patch(`/auth-files?name=${encodeURIComponent(name)}`, updates);
  }
};
