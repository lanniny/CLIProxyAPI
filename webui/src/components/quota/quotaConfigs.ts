...
// Kiro quota configuration
// Kiro uses AWS CodeWhisperer - no public quota API available
// Just display available models based on credential info
const fetchKiroQuota = async (
  file: AuthFileItem,
  t: TFunction
): Promise<KiroQuotaModel[]> => {
  const rawAuthIndex = file['auth_index'] ?? file.authIndex;
  const authIndex = normalizeAuthIndexValue(rawAuthIndex);
  if (!authIndex) {
  ...
  // Kiro doesn't have a public quota API like Codex or Antigravity
  // Instead, we display the default available models for Kiro credentials
  const defaultModels: KiroQuotaModel[] = [
    { id: 'claude-sonnet-4', name: 'Claude Sonnet 4', available: true },
    { id: 'claude-sonnet-4-20250514', name: 'Claude Sonnet 4 (20250514)', available: true },
...
const renderKiroItems = (
  ...
  if (models.length === 0) {
    return h('div', { className: styleMap.quotaMessage }, t('kiro_quota.empty_models'));
  }
  ...
  return models.map((model) => {
    return h(
      ...
      h(
        'div',
        { className: styleMap.quotaRowHeader },
        h('span', { className: styleMap.quotaModel, title: model.id }, model.name),
        h(
          'div',
          { className: styleMap.quotaMeta },
          h(
            'span',
            {
              className: model.available ? styleMap.quotaPercent : styleMap.quotaReset,
              style: { color: model.available ? 'var(--success-color)' : 'var(--danger-color)' }
            },
            model.available ? t('kiro_quota.available') : t('kiro_quota.unavailable')
          )
        )
      )
    );
...
export const KIRO_CONFIG: QuotaConfig<KiroQuotaState, KiroQuotaModel[]> = {
  type: 'kiro',
